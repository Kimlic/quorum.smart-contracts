...
const cleanupDeployedConfig = function (network) {
  if (!isConfigLoaded) loadDeployedConfigIntoCache()
  if (network.length < 0) throw "network is empty"
...
const getNetworkDeployedConfig = (network) => {
  if (!isConfigLoaded) loadDeployedConfigIntoCache()
  if (network.length < 0) throw "network is empty"
  if (!deployedConfig[network]) deployedConfig[network] = {}
...
const saveDeployedConfig = () => {
  ...
  if (!isConfigLoaded) throw "Config not loaded! Use loadDeployedConfigIntoCache method first."
